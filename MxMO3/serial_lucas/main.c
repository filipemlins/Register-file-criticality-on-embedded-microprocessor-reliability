      ...
      char buffer[BUFFER_SIZE];
		...
		if(ReadFile(hSerial, buffer, BUFFER_SIZE-1, &dwBytesRead, NULL))
		{
            ...
            while(ptr != NULL)
            {
			   ...
			   else if(strstr(ptr, "0;") || strstr(ptr, "SUCCESS;") || isdigit(*ptr) )
			   {
 			   	 printTime(logs);
                  fprintf(logs, ptr);
                  fprintf(logs, "\n\r");
                 ...
                 else
               {
                  ...
                  fprintf(logs, "\n\r");
                  printf("%s\n\r", ptr);
                  if( mustResetBoard == TRUE){
                  value = 1;
                 }
               }
               
               if(mustResetBoard && value == 1)
               {
			      mustResetBoard = FALSE;
			      value = 0;
			      CloseHandle(hSerial);
                  openConnection(argv[1], SLEEPING_TIME);
                  readback();
                  hardResetBoard();
